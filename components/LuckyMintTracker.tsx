...
export function LuckyMintTracker() {
  ...
  const verifyWinners = useCallback(async (winners: LuckyWinner[]) => {
    ...
    try {
      ...
      // ✅ FIXED: Match the correct prize amounts
  ...
  return (
    <div className="lucky-mint-tracker">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-2xl font-bold text-cyber-text-main">
          🎰 Lucky Mint Winners
        </h2>
        <div className="flex items-center gap-2">
          {drandRound && (
            <button
              ...
              className="text-xs text-cyber-primary hover:text-cyber-hover hover:underline flex items-center gap-1"
      ...
      <div className="mb-6 p-4 bg-yellow-100 dark:bg-yellow-100/30 border border-yellow-600 dark:border-yellow-500 rounded-lg">
        ...
        <ul className="space-y-1 text-sm text-cyber-text-main">
          ...
          <li>• 1 super lucky minter gets <strong>5,000 $DEGEN + 25,000,000 $MYU</strong></li>
      ...
      {userWins.length > 0 && (
        <div className="mb-6 p-4 bg-green-100 dark:bg-green-900/30 border border-green-600 dark:border-green-500 rounded-lg">
          <h3 className="font-bold mb-2 text-cyber-text-main">
            🎉 Your Wins: {userWins.length}
          </h3>
          {userWins.map((win, idx) => (
            <div key={idx} className="text-sm text-cyber-text-main mb-1">
              ...
              <a
                ...
                className="text-cyber-primary hover:text-cyber-hover hover:underline ml-2 text-xs"
              >
                tx
              </a>
            </div>
      ...
      {loading && luckyWinners.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="mt-2 text-cyber-text-muted">Loading winners...</p>
        </div>
      ) : luckyWinners.length > 0 ? (
        <div className="space-y-3">
          <h3 className="font-bold text-cyber-text-main">
            Recent Winners ({luckyWinners.length})
          </h3>
          {luckyWinners.slice(0, 10).map((winner, idx) => (
            <div
              key={`${winner.address}-${winner.timestamp}-${idx}`}
              className="flex items-center justify-between p-3 bg-cyber-bg-light rounded-lg border border-cyber-border transition-all hover:shadow-md"
            >
              <div className="flex items-center">
                ...
                <div>
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-cyber-primary hover:text-cyber-hover hover:underline font-mono text-sm"
                    title={winner.address}
                  >
                    {winner.displayAddress || formatAddress(winner.address)}
                  </a>
                  <div className="text-xs text-cyber-text-muted flex items-center gap-2">
              ...
              <div className="text-right">
                <div className="font-bold text-cyber-text-main">
          ...
          {luckyWinners.length > 10 && (
            <p className="text-center text-sm text-cyber-text-muted pt-2">
